...
+
[.boxshadow]
image:pipeline/classic-ui-new-item.png[alt = "Jenkins Classic UI left column showing options: New Item, Build History, Project Relationship, Check File Fingerprint, Manage Jenkins and My Views.",width=35%]
...
* Defining a Pipeline through the classic UI is convenient for testing Pipeline
  ...
  above, unlike ``Jenkinsfile``s you define in source control
...
https://issue-redirect.jenkins.io/issue/40550
////
...
Once you have defined and run your Pipeline, you can visualize its execution using the link:https://plugins.jenkins.io/pipeline-graph-view/[Pipeline Graph View plugin]. 
This actively maintained plugin provides a modern, detailed view of your Pipeline's progress, including:

1. A graphical representation of all stages and parallel branches.
2. Support for visualizing nested `parallel` stages.
3. In-line access to step-by-step logs.
4. Ability to restart the Pipeline from specific stages.

To take full advantage of these features, ensure the Pipeline Graph View plugin is installed on your Jenkins controller. You can read more about it in the link:/blog/2025/05/02/pipeline-graph-view/[Pipeline Graph View blog post].
