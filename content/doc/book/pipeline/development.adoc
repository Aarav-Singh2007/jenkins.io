...
The link:https://plugins.jenkins.io/pipeline-graph-view/[Pipeline Graph View plugin] provides a modern, detailed structural view of all the stages, parallel branches, and steps in a Pipeline.
While it does not edit pipelines, it is an invaluable visualization tool during development to ensure that your Pipeline's structure is correct and to monitor its execution step-by-step.
...
See link:https://issue-redirect.jenkins.io/issue/37589[JENKINS-37589]
...
See link:https://issue-redirect.jenkins.io/issue/36453[JENKINS-36453]
...
Pre-commit hooks allow you to automatically validate Jenkinsfiles before they are committed to version control, helping to catch syntax errors early in the development workflow.

=== jenkinsfilelint

The link:https://github.com/jenkinsci/jenkinsfilelint[jenkinsfilelint] pre-commit hook validates Jenkinsfiles before commit by using the <<linter,Pipeline Linter API>> of a Jenkins controller.
This can help teams catch Jenkinsfile syntax errors in local development and pull request workflows before they reach CI.
...
repos:
  - repo: https://github.com/jenkinsci/jenkinsfilelint
    rev: v1.4.0  # use the latest release
    hooks:
      - id: jenkinsfilelint
----

The hook connects to your Jenkins controller using the following environment variables:

* `JENKINS_URL` - the root URL of your Jenkins controller
